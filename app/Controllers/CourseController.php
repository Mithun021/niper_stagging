        ...
        if ($this->request->is("get")) {
            ...
            foreach ($course_id as $key => $value) {
                if (!isset($credit_score[$key]) || $credit_score[$key] === '') {
                    return redirect()->to('admin/assignCourseList')->with('status', '<div class="alert alert-danger" role="alert"> Credit score for course ' . $value . ' cannot be empty </div>');
                }
                ...
                $data = [
                    ...
                    'credits' => $credit_score[$key],  // Pass the credit score (which could be 0)
                    'upload_by' => $loggeduserId
                ];
